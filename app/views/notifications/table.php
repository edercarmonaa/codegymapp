                ...
                <tr>
                    ...
                    <td class="text-end">
                        <?php if ((int) $notification['is_read'] === 0): ?>
                            <form class="d-inline" action="/api/notifications/mark-read" method="post" data-api-form>
                                <?= csrf_field() ?>
                                <input type="hidden" name="id" value="<?= e((string) $notification['id']) ?>">
                                <button class="btn btn-sm btn-outline-primary">Marcar leída</button>
                            </form>
                        <?php else: ?>
                            <form class="d-inline" action="/api/notifications/delete" method="post" data-api-form data-confirm="¿Eliminar esta notificación del historial?">
                                <?= csrf_field() ?>
                                <input type="hidden" name="id" value="<?= e((string) $notification['id']) ?>">
